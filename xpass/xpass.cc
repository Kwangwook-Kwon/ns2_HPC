...
seq_t XPassAgent::recv_credit_mpath(Packet *pkt, int remain_bytes)
{
	...
	seq_t send_datalen = min(max_segment(), remain_bytes);
...
void XPassAgent::recv_data(Packet *pkt)
{
	...
	if (distance < 0)
	{
		...
		fprintf(stderr, "ERROR: Credit Sequence number is reverted. %d %d\n", xph->credit_seq(), c_recv_next_);
		exit(1);
	}
...
void XPassAgent::send_credit()
{
	...
	double delay;
	int ret = credit_feedback_control();
	int i = 0;

	if (ret == 1) {
		send_credit_timer_.force_cancel();

		parent_->act_sub_num_ = 0;

		for (i=0; i< parent_->sub_num_ ; i++){
			parent_->subflows_[i].xpass_->set_deactive(); 		
//			parent_->mp_sender_state_ = MP_SENDER_CREDIT_REQUEST_SENT;
			parent_->subflows_[i].xpass_->send_one_credit(); //(construct_credit(), 0);		
		}
	}
	else {
		// send credit.
...
int XPassAgent::credit_feedback_control()
{
	int ret = 0;
	if (rtt_ <= 0.0)
	{
	...
	{
		congestion_++;
		if(congestion_ >= 5){ // 5 is threshold
			ret = 1;
			congestion_ = 0;
			fct_timer_.force_cancel();
			send_credit_timer_.force_cancel();
		...
		{
			cur_credit_rate_ = cur_credit_rate_ = (int)(avg_credit_size() * (credit_total_ - credit_dropped_) / ((now() - last_credit_rate_update_)) * (1.0 + target_loss));
			//			cur_credit_rate_ = (int)(avg_credit_size() * (credit_total_ - credit_dropped_) / ((now() - last_credit_rate_update_)*2) * (1.0 + target_loss));

			//cur_credit_rate_ = min(temp, cur_credit_rate_/2);
		}
